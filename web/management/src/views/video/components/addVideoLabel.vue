<template>
  <el-dialog title="添加视频标签" :before-close="handleClose" :visible.sync="ishide" class="add">
...
<script type="text/javascript">
  ...
  export default {
    data() {
      const validateName = (rule, value, callback) => {
        if (value === '') {
          ...
          callback(new Error('标签名长度不能大于10位'));
        } else {
          callback();
        }
      };
    ...
    methods: {
      ...
      // 添加视频标签
      handleAddLabel(sortorder, name) {
        this.addLabel(sortorder, name)
      },

      async addLabel(sortorder, name) {
        const res = await addVideoLabel({
          label_name: name,
          sortorder: parseInt(sortorder),
        });
        console.log(res);
        if (res.code === 200) {
          this.$message.success("视频标签添加成功")
          this.handleClose()
        } else {
      ...
      submitForm() {
        this.$refs.ruleForm.validate(async valid => {
          if (valid) {
            ...
            this.handleAddLabel(this.form.sortorder, this.form.name)
          } else {
