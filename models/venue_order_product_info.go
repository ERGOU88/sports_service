...
type VenueOrderProductInfo struct {
	...
	OrderType      int    `json:"order_type" xorm:"not null comment('1001 场馆预约 2001 购买月卡 2002 购买季卡 2003 购买年卡 2004 体验券 3001 私教（教练）订单 3002 课程订单 4001 充值订单') INT(8)"`
	...
	Status         int    `json:"status" xorm:"not null comment('0 待支付 1 订单超时/未支付 2 已支付 3 已完成 4 已取消  5 退款中 6 已退款 7 软删除') TINYINT(2)"`
	VenueId        int64  `json:"venue_id" xorm:"not null default 1 comment('场馆id') BIGINT(20)"`
}
