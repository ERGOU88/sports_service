...
type VenueVipInfo struct {
	...
	UserId   string `json:"user_id" xorm:"not null comment('用户id') unique(user_id) VARCHAR(60)"`
	Level    int    `json:"level" xorm:"not null default 0 comment('会员等级 预留字段') INT(2)"`
	VenueId  int64  `json:"venue_id" xorm:"not null comment('场馆id') unique(user_id) BIGINT(20)"`
