...
type SystemMessage struct {
	...
	SystemContent string `json:"system_content" xorm:"not null comment('通知内容') VARCHAR(255)"`
	SystemId      int64  `json:"system_id" xorm:"not null pk autoincr comment('系统通知ID') BIGINT(20)"`
	SystemTopic   string `json:"system_topic" xorm:"not null comment('通知标题') VARCHAR(60)"`
}
