...
type SystemMessage struct {
	...
	SendType      int    `json:"send_type" xorm:"not null default 0 comment('0.默认为系统通知 1 活动通知 ') index TINYINT(1)"`
	Extra         string `json:"extra" xorm:"not null default ' ' comment('附件内容 例如：奖励') VARCHAR(1024)"`
	Status        int    `json:"status" xorm:"not null default 0 comment('0 未读 1 已读  默认未读') index TINYINT(1)"`
