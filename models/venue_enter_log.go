...
type VenueEnterLog struct {
	...
	UserId        string `json:"user_id" xorm:"not null comment('用户user id') index(user_id) index(user_id_2) VARCHAR(255)"`
	...
	VenueId       int64  `json:"venue_id" xorm:"not null default 0 comment('场馆ID') index(user_id) index(user_id_2) BIGINT(20)"`
	...
	DtEnter       int    `json:"dt_enter" xorm:"not null default 0 comment('入场时间') INT(11)"`
