...
type VenueAppointmentRecord struct {
	...
	OrderProductId  int64  `json:"order_product_id" xorm:"default 0 comment('订单商品ID') index BIGINT(20)"`
	...
	AppointmentId   int64  `json:"appointment_id" xorm:"comment('预约配置表id') index(appointment_id) BIGINT(20)"`
	...
	StartTm         int    `json:"start_tm" xorm:"not null default 0 comment('预约开始时间') index(appointment_id) INT(11)"`
